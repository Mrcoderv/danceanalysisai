...
with open("dance_tracking_data.csv", mode="w", newline="") as file:
    ...
    beat_idx = 0
    last_beat_time = 0
    stickman_beat_pose = 0  # 0: left tap, 1: right tap, 2: prayer, 3: clap
    ...
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            # If camera fails, use a black frame for stickman animation
            h, w = 480, 640
        ...
        if beat_idx < len(beats) and abs(elapsed - beats[beat_idx]) < 0.2:
            ...
            stickman_beat_pose = (stickman_beat_pose + 1) % 4  # Cycle through 4 poses
            beat_idx += 1

        # Always show beat indicator if on beat
        if on_beat:
        ...
        # Style-specific stickman pose cycles
        style_pose_cycles = {
            'bhajan': [
        ...
        # --- Remove hint text for Bhajan ---
        # (No text drawn)

        # --- Resize combined image to fit the screen if maximized ---
        import ctypes
        ...
        if key == ord("q"):
            break
        # Style selection: keys 49-53 (ASCII for '1'-'5')
        if 49 <= key <= 49 + 4:
            style_idx = key - 49
